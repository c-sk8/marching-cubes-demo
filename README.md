# Marching Cubes Demo

A real-time marching cubes surface visualiser with progressive slice-by-slice rendering and interactive controls.

🔗 **Live Demo:** http://c-sk8.github.io/marching-cubes-demo

---

## Features

- Real-time **slice-based surface generation** (watch the surface build itself)
- Runs in any modern browser — including phones and tablets
- Adjustable **speed and detail levels**
- Multiple **field functions** (Decocube, torus variants, etc.)
- Various **colour modes and surface patterns**
- Optimised for performance using cached field evaluation and bounding boxes

---

## Controls

Click the on-screen buttons or use the keyboard:

| Key | Action |
| --- | --- |
| Left Arrow | Previous field function |
| Right Arrow | Next field function |
| Up Arrow | Next colour / pattern mode |
| Down Arrow | Previous colour / pattern mode |
| / | Toggle flat shading / smooth normals |
| W or + | Zoom in |
| S or - | Zoom out |
| h or H | Hide and show controls |

---

## Acknowledgements

This project would have taken much longer without help from:

- https://chatgpt.com  
- https://threejs.org
